...
fun Chip(
    ...
    InfiniteBorderAnimationView(
        ...
        // Material's 48dp minimum touch target stays ON. It used to be switched off right here with
        // `CompositionLocalProvider(LocalMinimumInteractiveComponentSize provides Dp.Unspecified)`,
        // which left the mood filters — the primary control on Home — about 29dp tall, well under
        // the platform floor, so the smallest ones were genuinely hard to hit and an accessibility
        // scanner measured them as undersized. Only the LAYOUT box grows to 48dp: the pill still
        // draws at its own size, centred, and the extra height is touch target only — the visual
        // chip is unchanged and selection still costs no width, so nothing reflows under the finger.
        ElevatedFilterChip(
            shape = CircleShape,
            ...
            label = {
                // Ellipsis, not the default clip: at font scale 1.3 a long label in a long locale
                // (German "Zum Einschlafen", Vietnamese "Thư giãn nhẹ nhàng") runs out of pill and
                // was being cut mid-word with nothing to say it had been cut.
